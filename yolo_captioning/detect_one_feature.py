...
from  model_one_feature import EncoderCNN, AttnDecoderRNN
...
def arg_parse():
    ...
    """
    base_path=r'/data/antonio/images_data/images/'
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch,stdout):
    ...
    for i, (imgs, caps, caplens,allcaps) in enumerate(train_loader):
        ...
        imgs = imgs.to(device)
        caps = caps.to(device)
        prediction, feature= encoder(imgs)

        detection_box = non_max_suppression(prediction, 80, args.conf_thres, args.nms_thres)
        feature=getresult(detection_box, feature)

        # alphas=(32*23*196)
        scores, caps_sorted, decode_lengths, alphas = decoder(feature,caps, caplens,detection_box)
...
def getresult(detection_box,feature):
    """
    ...
    for img_i, detection in enumerate(detection_box):
        ...
        if  detection is None:
            # feature_max_box = feature[img_i, :, x1:x2, y1:y2]
            # features,max=feature_cal(feature_max_box, features, feature, max)
            continue
        ...
        feature[img_i,:,x1:x2,y1:y2]=feature[img_i,:,x1:x2,y1:y2]+feature[img_i,:,x1:x2,y1:y2]
        #feature_max_box=feature[img_i,:,x1:x2,y1:y2]
    return feature
    #     features,max=feature_cal(feature_max_box, features,max)
...
def feature_cal(feature_max_box,features,max):
